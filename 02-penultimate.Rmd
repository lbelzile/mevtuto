...
\cite{Smith:1987} shows that, for any $x \in \{y:1+\xi_\infty y >0\}$, there exists $z$ such that 
\begin{align*}
   \frac{-\log[F\{v+xs(v)\}]}{-\log\{F(v)\}} = \left\{1+xs'(z)\right\}^{-1/s'(z)}, \qquad v < z < v+xs(v).
\end{align*}
For each $n \geq 1$, setting $v=b_n$ and $a_n=s(b_n)$ yields 
\begin{align*}
   F^n(a_nx+b_n)=\exp\left[-\left\{1+s'(z)x\right\}^{-1/s'(z)}\right] + \bigO(n^{-1})
\end{align*}
for $z \in [\min(a_nx+b_n, b_n), \max(a_nx+b_n, b_n)]$, depending on the support of $F$. The ultimate approximation replaces $s'(z)$ by $s'(x^*)=\xi_{\infty}$, whereas \citeauthor{Smith:1987} suggests instead suggests taking $s'(b_n)$, which is closer to  $s'(a_nx+b_n)$ than is $s'(x^*)$. 
