...
Let $\mathcal{S}$ be a compact subset of $\R^d$. 
The de Haan spectral representation [@deHaan:1984] of simple max-stable processes is [@Schlather:2003]
\[Z(\bs{s}) = \max_{i \in \N} \zeta_i W_i(\bs{s}), \qquad \bs{s} \in \mathcal{S},\]
where $\bs{W}$ is a stochastic process satisfying $\mathrm{E}\{W(\bs{s})_{+}\}=1$ for any $\bs{s} \in \mathcal{S}$ and $\{\zeta_i\}_{i \in \N}$ is a Poisson point process with intensity function $\zeta^{-2} \mathrm{d} \zeta$. Max-stable processes arise as the pointwise maximum of an infinite collection of random functions $\varphi_i = \zeta_i \bs{W}_i$. 

The distribution function of a $D$-dimensional max-stable vector can be defined in terms of a measure $\Lambda$, termed exponent measure, defined on $\R^D \setminus\{\bs{0}_D\}$ viz.
\[\Pr(\bs{Z} \leq \bs{z}) = \exp\{\Lambda([-\bs{\infty}, \bs{z}]^\mathrm{c})\}, \quad \bs{z} > \bs{0}_D.\]
It follows from max-stability that the exponent measure is homogeneous of order $-1$. 

## $\Ri$-Pareto processes and generalizations 


Let $\mathcal{F}^+$ denote the set of continuous non-negative non-null functions. A risk functional $\Ri:\mathcal{F}^+ \to [0, \infty)$ is a continuous functional which is homogeneous of order $1$, i.e., $\Ri(tf) = t\Ri(f)$ for $f \in \mathcal{F}^+$ and $t>0$. Examples include maxima, $\Ri(f) = \max_{j=1}^D f(\bs{s}_j)$, minima, $\Ri(f) = \min_{j=1}^D f(\bs{s}_j)$, sums
$\Ri(f) = \sum_{j=1}^D f(\bs{s}_j)$ and projections $\Ri(f) = X(\bs{s}_j)$. A $\Ri$-Pareto process admits the stochastic representation [@Ferreira:2014][@Dombry/Ribatet:2015]
 \begin{align}
  \mathsf{X} = P \frac{\mathsf{S}}{\Ri(\mathsf{S})},
 \end{align}
for $P$ a unit Pareto random variable with survival function $\Pr(P>x) = x^{-\alpha}$ for $x \geq 1$ and a stochastic process $\mathsf{S}$, independent of $P$,  with sample paths in $\mathbb{S}_{\ang} = \{f \in \mathcal{F}^+ \setminus \cone_{\Ri}: \|f\|_{\ang} = 1\}$ and where $\cone_{\Ri} = \{g \in \mathcal{F}^+: \Ri(x)=0\}$ and $\|\cdot\|_{\ang}$ is a norm. $\Ri$-Pareto processes come about when looking at the limit distribution of $\Ri$-exceedances, i.e. $\{f: \Ri(f) >u\}$ for a large threshold $u$, assuming unit Frechet margins. This can be achieved in practice by standardizing the margins, but with the drawback that the exceedances are defined on the transformed scale.

@deFondeville:thesis lifts this assumption and define generalized $\Ri$-Pareto processes, allowing for different location and scale marginal parameters (but with a common shape parameter). 
The risk region in which extreme observations lie is
\begin{align*}
\mathcal{A}_u= 
\left\{f \in \mathcal{F}^+ \setminus \mathcal{C}_{\Ri}: \Ri \left( \bs{\tau} \frac{f^\xi-1}{\xi}+\bs{\eta}\right) \geq u\right\},
\end{align*}
where $\bs{\eta}$ is a location function and $\bs{\tau}$ is a scale function. 
The probability measure of $\mathsf{Z}^*$ over $\mathcal{A}_u$ is  $\Lambda(\cdot)/\Lambda\{\mathcal{A}_u\}$.
The stochastic representation of the generalized $\Ri$-Pareto vector is 
\begin{align}
	Z(\bs{s}) = \tau(\bs{s}) \frac{X(\bs{s})^{\xi}-1}{\xi}+ \eta(\bs{s}), 
\end{align}
where $X(\bs{s})$ is a $\Ri$-Pareto process. When $\Ri$ is $\max$, the finite-dimensional distributions are multivariate generalized Pareto [@Rootzen:2017].

## Algorithms

The original goal of the `mev` package was to implement the algorithm of @Dombry:2016,  in order to perform exact simulation from max-stable vectors by sampling each extremal function in turn. Since then, other algorithms for simulating from standard $\Ri$-Pareto processes and generalized $\Ri$-Pareto processes have been added to the package. These are based on accept-reject method or composition sampling and can be slow in high dimensions. Note that packages such as `RandomFields` and `SpatialExtremes` are tailored for spatial processes and include more efficient algorithms to sample the (log-)Gaussian components appearing in the Brown--Resnick and extremal Student models, which makes them typically faster. The vignette provides details about the parametrization of the models implemented in `mev`.
...
- `rparp` for simulating $\Ri$-Pareto processes through accept-reject;
- `rparpcs` for simulating $\Ri$-Pareto processes using composition sampling, e.g. @Ho/Dombry:2019
- `rgparp` for simulating generalized $\Ri$-Pareto processes using accept-reject. 

This approach is only available for Brown--Resnick and extremal Student processes and is more computationally intensive since it requires computing numerically mixture weights. However, these need only be computed once and this approach is more efficient for simlating large samples, particularly in cases where the acceptance rate is very low (notably if $R(\bs{X})=\min_{j=1}^DX_j$).
...
\end{align}
 ...
 $z_{ij}^{\mathrm{p}} = 1/1-(u_{ij})$. If we map the  random vector $\boldsymbol{Z}^{\mathrm{p}}$ to pseudo-polar coordinates,  $\bs{Z}^{\mathrm{p}} \mapsto (R, \bs{\Omega})$ from $\R^D_{+} \to (0, \infty) \times \mathbb{S}_1$, where $\mathbb{S}_1=\{ \bs{\omega} \in \R^D_{+}: \|\bs{\omega}\|_1=1\}$ is the $\ell_1$-simplex. The measure $\Lambda$ factorizes as a product measure $\Lambda(\d \bs{z}) = D \zeta^{-2}\d \zeta \rho_1(\d \bs{\omega})$ with angular measure $\rho_1$, a probability measure satisfying the moment constraint  $D\int_{\mathbb{S}_1} \omega_j \rho_1(\d \bs{\omega})=1$ for $j=1, \ldots, D$;  this moment constraint holds for any $\rho_1$, whereas it will be measure dependent if the radial measure is not $\|\cdot \|_1$  [@Einmahl:2009].
 We can thus create an approximate sample $\bs{\omega}$ from the angular measure $\rho_1$ and estimate its distribution non-parametrically as 
...
dirangmeas <- angmeasdir(samp, th = 0.5)
```
...
## Generalized $\Ri$-Pareto processes for functional threshold exceedances


We simulate a spatial generalized $\Ri$-Pareto process.
